...
export function forwardPrometheusService(
    ...
    // pulumi.all([service]).apply(([service]) => console.log("service: %o", service.metadata));
    // pulumi.all([deployment]).apply(([deployment]) => console.log("deployment: %o", deployment.urn));
    // console.log("opts.localPort: [%o]", opts.localPort)
    // console.log("opts.targetPort: [%o]", opts.targetPort)

    return pulumi.all([service]).apply(([s]) => pulumi.all([s.metadata])).apply(([meta]) => {
        console.log("meta.name: [%o]", meta.name)
    ...
    function pollRecursive(): Promise<string> {
        return getHttpLatency(url).then(bodyText => {
            ...
            for (const result of body.data.result) {
                const quantile = parseFloat(result.metric["quantile"]);
                microseconds = result.value[1];
                // console.log("[%o] for>> timedOut=[%o], quantile=[%o], microseconds=[%o], threshold=[%o]", now, timedOut, quantile, parseFloat(microseconds), opts.thresholdMicroseconds);
                ...
                if (quantile === opts.quantile) {
                    // console.log("[%o] timedOut=[%o], quantile=[%o], microseconds=[%o], threshold=[%o]", now, timedOut, quantile, parseFloat(microseconds), opts.thresholdMicroseconds);
            ...
            console.log("[%o] EXCEPTION: timedOut=[%o], microseconds=[%o], threshold=[%o]", now, timedOut, parseFloat(microseconds), opts.thresholdMicroseconds);
            throw new Error(
